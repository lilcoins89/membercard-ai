...
export async function POST(req: Request) {
  try {
    const { shipment_id, signature } = await req.json();
    if (!shipment_id || typeof signature !== "string" || signature.length < 80) return NextResponse.json({ ok: false, error: "A valid payment signature is required." }, { status: 400 });
    ...
    await db.execute(sql`UPDATE public.shipment_orders SET payment_status = 'paid', solana_signature = ${signature}, paid_at = now(), updated_at = now() WHERE id = ${order.id} AND payment_status = 'pending'`);
