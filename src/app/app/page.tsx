...
export default async function DashboardPage() {
  ...
  return <main className="min-h-dvh bg-background">
    ...
    <section className="mx-auto max-w-6xl space-y-8 px-5 pb-16 lg:px-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]"><section className="rounded-2xl border border-border bg-card p-5 md:p-7"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-primary">Latest card</p><h2 className="mt-2 font-display text-2xl">Your membership card</h2></div><Button asChild variant="ghost" size="sm"><Link href="/create">Edit <ArrowRight className="size-4" /></Link></Button></div>{card ? <DashboardCardPreview /> : <div className="mt-8 rounded-xl bg-secondary/50 p-5 text-sm text-muted-foreground">No saved membership card yet. Create your first card to activate this workspace.</div>}</section><section className="rounded-2xl border border-border bg-card p-5 md:p-7"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-primary">Delivery</p><h2 className="mt-2 font-display text-2xl">Recent shipments</h2></div><Truck className="size-5 text-primary" /></div>{shipments.length === 0 ? <div className="mt-10 rounded-xl bg-secondary/50 p-5 text-sm text-muted-foreground">No shipments yet. Create a card and send one to your members.</div> : <div className="mt-6 space-y-3">{shipments.map((shipment) => <div key={String(shipment.id)} className="flex items-center justify-between gap-3 rounded-xl border border-border p-4"><div><p className="font-medium">{shipment.recipient_name}</p><p className="text-xs text-muted-foreground">{shipment.country}</p></div><span className="rounded-full bg-secondary px-3 py-1 text-xs capitalize">{String(shipment.fulfillment_status).replaceAll("_", " ")}</span></div>)}</div>}<Button asChild className="mt-6 w-full" variant="outline"><Link href="/shipments">View delivery timeline <ArrowRight className="size-4" /></Link></Button></section></div>
    </section>
  </main>;
}
